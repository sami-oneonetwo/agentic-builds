"""buildings.py - settlers-tall building kit (from studio-c; hut shadows fall away from the sun) (SETTLEMENT top-down, 3/4 view).
...
from tiles import _D, _mix, _scale, OUTLINE, T, FLAT  # noqa: E402
...
def hut(name: str, w_tiles: int = 1, lit: bool = False, sun: Tuple[float, float] = (-0.75, -0.66)) -> Tuple[np.ndarray, Tuple[int, int]]:
    col = colours(name)
    fw = T * w_tiles
    ...
    kind = _h(name, 1) % 3
    d.ellipse(ox + fw / 2.0 - sun[0] * 5, oy + T - 3 - sun[1] * 1.5, fw / 2.0 + 5, 5.5, (20, 24, 12, 80))   # ground shadow away from the sun
...
if __name__ == "__main__":
    ...
    kit_image(names).save("/Users/sandy/Workspace/agentic-builds/kick-live/docs/art/settlers-tall/building_kit.png")
    print("ok")
