"""hud.py - settlers-tall HUD (from studio-c; name pills carry a portrait of the settler instead of a dot) + drawing helpers (SETTLEMENT top-down).
...
def pill(text: str, dot: Optional[Tuple[int, int, int]] = None, tail: bool = True, portrait: Optional[Image.Image] = None) -> Image.Image:
    """Show-on-speak name pill for the world: dark rounded pill, cream text (20 px), the settler's portrait (or a dot
    in their colour) and a small tail pointing down at them. Returns RGBA."""
    f = font("bold", SIZES["pill"])
    tw = text_w(text, f)
    dw = 34 if portrait is not None else (22 if dot else 0)
    w, h = tw + dw + 24, 32 + (6 if tail else 0)
    im = Image.new("RGBA", (w, h), (0, 0, 0, 0))
    d = ImageDraw.Draw(im)
    d.rounded_rectangle([0, 0, w - 1, 31], radius=14, fill=COLOURS["pill_bg"], outline=COLOURS["panel_border"], width=2)
    if tail:
        d.polygon([(w // 2 - 6, 30), (w // 2 + 6, 30), (w // 2, 37)], fill=COLOURS["panel_border"])
        d.polygon([(w // 2 - 4, 29), (w // 2 + 4, 29), (w // 2, 34)], fill=COLOURS["pill_bg"][:3])
    x = 12
    if portrait is not None:
        if dot:
            d.ellipse([x - 3, 3, x + 27, 29], fill=dot + (255,))
        pm = portrait.resize((26, 26), Image.Resampling.LANCZOS) if portrait.size != (26, 26) else portrait
        im.paste(pm, (x - 1, 3), pm)
        x += dw
    elif dot:
        d.ellipse([x, 9, x + 14, 23], fill=dot, outline=COLOURS["panel_border"], width=2)
        x += dw
    d.text((x, 4), text, font=f, fill=COLOURS["cream"])
    return im
...
def style_sheet_image() -> Image.Image:
    ...
    header(img, [("atleastonce", COLOURS["muted"]), ("SETTLEMENT · v0.7", COLOURS["cream"])], "14 SETTLED · 12 AWAKE",
...
if __name__ == "__main__":
    style_sheet_image().save("/Users/sandy/Workspace/agentic-builds/kick-live/docs/art/settlers-tall/hud_style.png")
    print("ok")
