"""render_mockup.py - studio-c SETTLEMENT mockups rendered FROM the generators, the way the compositor would.
...
class Baker:
    ...
    def tint(self, bg: np.ndarray) -> np.ndarray:
        ...
        if self.w.evening:
            warm = np.array((255, 168, 92), np.float32)
            f = f * 0.86 * (1 - 0.16) + warm * 0.16 * (f / 255.0) * 1.9
            # cooler in the lower-right (the sea side is already in the shade of dusk)
            grad = (self.w.PX / WW * 0.5 + self.w.PY / WH * 0.5)[..., None]
            f = f * (1.0 - 0.10 * grad) + np.array((40, 30, 80), np.float32) * 0.10 * grad
        else:
...
def place_creatures(world: World) -> List[Dict]:
    ...
    L = [
        ...
        {"name": "kai_dnb", "frame": "speak", "pos": (cx - 70, cy + 20), "speech": "B! the well goes by the plaza"},
        {"name": "sami.exe", "frame": "speak", "pos": on_path(len(huts), 0.55, 0, 12), "speech": "we need a bridge over it"},
        {"name": "noor.wav", "frame": "speak", "pos": farm(0, -8, 4), "speech": "planting by the water, who's with me"},
        ...
        {"name": "xX_tobi_Xx", "frame": "walk1", "pos": on_path(len(huts), 0.25, 0, -10), "speech": None},
        {"name": "lowkeyjord", "frame": "sleep", "pos": door("lowkeyjord", -22, 6), "speech": None},
        {"name": "tinytash", "frame": "emote0", "pos": (cx + 66, cy + 34), "speech": None},
        {"name": "atleastonce", "frame": "emote1", "pos": (cx - 6, cy + 58), "speech": None},
        {"name": "pixel_dude", "frame": "hop", "pos": on_path(len(huts) + 1, 0.7, 12, 0), "speech": None},
        {"name": "gg_nora", "frame": "idle0", "pos": farm(1, 30, -2), "speech": None},
        {"name": "hollowbyte", "frame": "idle1", "pos": farm(2, 40, -6), "speech": None},
        {"name": "june.mp4", "frame": "blink", "pos": (cx + 30, cy - 30), "speech": None},
    ]
    for c in L:
        x, y = c["pos"]
    ...
    def __init__(self, world: World, baker: Baker, actors: List[Dict]):
        ...
        for a in actors:
            if a.get("speech"):
                ...
                self.pills[a["name"]] = np.asarray(hud.pill("@" + a["name"], dot=pal["main"]))
    ...
    def compose(self, t: float) -> np.ndarray:
        ...
        for a in order:
            if a["name"] in self.pills:
                p = self.pills[a["name"]]
                ax, ay = creatures.anchor(a["tier"])
                sh = creatures.frame(a["name"], a["tier"], a["frame"]).shape[0]
                px = int(np.clip(int(a["pos"][0]) - p.shape[1] // 2, 4, WW - p.shape[1] - 4))
                py = max(2, int(a["pos"][1]) - ay - 6 - p.shape[0] + 8)
                blit(bg, p, px, py)
...
def hud_frame(world: World, frame_rgb: np.ndarray, scenario: str) -> Image.Image:
    ...
    busy = scenario == "busy"
    if busy:
        hud.header(img, [("atleastonce", C["muted"]), ("SETTLEMENT · v0.7 concept", C["cream"])], "14 SETTLED · 12 AWAKE",
                   [("LIVE · 7 watching", C["cream"])], [("NEXT EVENT 01:23", C["gold"])], progress=0.54)
    else:
        hud.header(img, [("atleastonce", C["muted"]), ("SETTLEMENT · v0.7 concept", C["cream"])], "2 SETTLED · 0 AWAKE",
    ...
    if busy:
        ...
        hud.caption(img, "concept render · names are examples · studio-c", "topdown · seed 4471 · 40x14 of 256x256 tiles · 18:07 evening")
    else:
        hud.panel(img, (g, y0, g + pw, y1), "COLONY", [
        ...
        hud.caption(img, "concept render · names are examples · studio-c", "topdown · seed 4471 · 40x14 of 256x256 tiles · 06:41 dawn")
    return img
...
if __name__ == "__main__":
    ...
    for i, n in enumerate(extra):
        fr.actors.append({"name": n, "frame": creatures.FRAMES[i % 10], "pos": (200 + i * 150, 300 + (i % 3) * 30),
