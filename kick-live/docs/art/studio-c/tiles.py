"""tiles.py - studio-c tile atlas generator (SETTLEMENT top-down).
...
def tree(size: int, variant: int, phase: int, season: float) -> np.ndarray:
    """Canopy cloud with a peeking trunk and a ground shadow; anchor = (W//2, H-4). Sway follows the wind phase."""
    R = TREE_R[size]
    W, H = int(R * 2.8), int(R * 3.0)
    d = _D(W, H)
    rng = _rng("tree", size, variant)
    cx, base_y = W / 2.0, H - 4.0
    sway = WIND_LEAN[phase] * (1.2 + 0.6 * size)
    # shadow
    d.ellipse(cx + 2, base_y, R * 0.9, R * 0.32, (20, 24, 12, 90))
    # trunk
    d.rect(cx - R * 0.16, base_y - R * 0.7, cx + R * 0.16, base_y, FLAT["trunk"], OUTLINE, 0.8, r=1.0)
    # canopy: a cluster of lobes
    lobes = []
    n = 4 + size
    for k in range(n):
        a = 2 * math.pi * k / n + rng.uniform(-0.3, 0.3)
        rr = R * rng.uniform(0.35, 0.55)
        lobes.append((cx + sway + math.cos(a) * R * 0.55, base_y - R * 1.15 + math.sin(a) * R * 0.45, rr))
    lobes.append((cx + sway, base_y - R * 1.15, R * 0.7))
    col = season_colour("tree", season)
    light = season_colour("tree_light", season)
    ow = 1.0 + 0.2 * size
    for (x, y, rr) in lobes:
        d.ellipse(x, y, rr, rr, OUTLINE, None)
    for (x, y, rr) in lobes:
        d.ellipse(x, y, rr - ow, rr - ow, col)
    for (x, y, rr) in lobes[:len(lobes) // 2 + 1]:
        d.ellipse(x - rr * 0.25, y - rr * 0.3, rr * 0.5, rr * 0.4, light)
    return d.out_rgba()


def tree_anchor(size: int) -> Tuple[int, int]:
    R = TREE_R[size]
    return int(R * 2.8) // 2, int(R * 3.0) - 4
...
if __name__ == "__main__":
    ...
    atlas_image(1.0).save("/Users/sandy/Workspace/agentic-builds/kick-live/docs/art/studio-c/tile_atlas.png")
