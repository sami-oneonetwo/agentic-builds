"""render_mockup.py - settlers-round SETTLEMENT mockups rendered FROM the generators, the way the compositor would.
Studio C's pipeline with people-shaped settlers, Studio A's dusk light and Studio B's ground richness.
            ...
            for tx in range(TX):
                ...
                if cc == C_FLOOR and r < 0.88:
                    ...
                    if not self.water[min(WH - 1, y), min(WW - 1, x)]:
                        objs.append(("bush", y, (x, y, int(hsh("bv", tx, ty) * 3))))
                    continue
                else:
                    continue
...
class Baker:
    ...
    def bake(self, wind: int, ripple: int) -> np.ndarray:
        ...
        for kind, _, data in self.objs:
            if kind == "tree":
                x, y, size, var = data
                spr = tiles.tree(size, var, wind, w.season)
                ax, ay = tiles.tree_anchor(size)
                blit(bg, spr, x - ax, y - ay)
            elif kind == "bush":
                x, y, var = data
                spr = tiles.bush(var, wind, w.season)
                blit(bg, spr, x - 10, y - 14)
            elif kind == "hut":
                spr, (dx, dy) = buildings.hut(data["name"], data["w"], lit)
    ...
    def tint(self, bg: np.ndarray) -> np.ndarray:
        ...
        if self.w.evening:
            warm = np.array((255, 172, 96), np.float32)
            f = f * 0.87 * (1 - 0.17) + warm * 0.17 * (f / 255.0) * 1.9
            # the sun sits top-left: a warm wash there, a cool violet shade toward the bottom-right (Studio A)
            grad = np.clip((self.w.PX / WW * 0.55 + self.w.PY / WH * 0.45), 0, 1)[..., None]
            f = f * (1.0 - 0.12 * grad) + np.array((44, 30, 84), np.float32) * 0.12 * grad
            sunw = np.clip(1.0 - np.hypot(self.w.PX - WW * 0.12, (self.w.PY + 40) * 1.3) / 1000.0, 0, 1)[..., None] ** 1.8 * 0.22
            f = f * (1 - sunw) + np.array((255, 206, 150), np.float32) * sunw
        else:
            ...
            # low sun from the top-right: a soft warm wash
...
def place_creatures(world: World) -> List[Dict]:
    ...
    L = [
        ...
        {"name": "luca_99", "frame": "hop", "pos": on_path(3, 0.5, 6, 0), "speech": None},
        ...
        {"name": "atleastonce", "frame": "wave", "pos": (cx - 6, cy + 58), "speech": None},
        ...
        {"name": "hollowbyte", "frame": "look", "pos": farm(2, 40, -6), "speech": None},
        {"name": "june.mp4", "frame": "look", "pos": (cx + 30, cy - 30), "speech": None},
    ]
    for c in L:
        x, y = c["pos"]
    ...
    for c in L:
        if c["frame"] == "look" and speakers:      # idle settlers turn their head toward the nearest speaker
            sp = min(speakers, key=lambda s_: math.hypot(s_["pos"][0] - c["pos"][0], s_["pos"][1] - c["pos"][1]))
            c["facing"] = 1 if sp["pos"][0] >= c["pos"][0] else -1
...
class Frame:
    def __init__(self, world: World, baker: Baker, actors: List[Dict]):
        ...
        for a in actors:
            if a.get("speech"):
                ...
                self.pills[a["name"]] = np.asarray(hud.pill("@" + a["name"], icon=creatures.head_icon(a["name"], 24), colour=pal["main"]))
        for a in actors:   # pre-warm the creature cache (what hatch does)
            for f in creatures.FRAMES:
                creatures.frame(a["name"], a["tier"], f)
    ...
    def compose(self, t: float) -> np.ndarray:
        ...
        for a in order:
            spr = creatures.frame(a["name"], a["tier"], a["frame"], a["facing"])
        ...
        for a in order:
            if a["name"] in self.pills:
                p = self.pills[a["name"]]
                top = int(a["pos"][1] - creatures.TIERS[a["tier"]]["h"] * (1.45 if a["frame"] == "hop" else 1.22))
                px = int(np.clip(int(a["pos"][0]) - p.shape[1] // 2, 4, WW - p.shape[1] - 4))
                py = max(2, top - p.shape[0] + 2)
                for _ in range(6):        # stack pills that would overlap an already placed one
                    hit = [q for q in placed if not (px + p.shape[1] < q[0] or q[0] + q[2] < px or py + p.shape[0] < q[1] or q[1] + q[3] < py)]
                    if not hit:
                        break
                    py = min(hit, key=lambda q: q[1])[1] - p.shape[0] - 2
                py = max(2, py)
                placed.append((px, py, p.shape[1], p.shape[0]))
                blit(bg, p, px, py)
...
def hud_frame(world: World, frame_rgb: np.ndarray, scenario: str) -> Image.Image:
    ...
    busy = scenario == "busy"
    if busy:
        hud.header(img, [("atleastonce", C["muted"]), ("SETTLEMENT · settlers-round", C["cream"])], "14 SETTLED · 12 AWAKE",
                   [("LIVE · 7 watching", C["cream"])], [("NEXT EVENT 01:23", C["gold"])], progress=0.54)
    else:
        hud.header(img, [("atleastonce", C["muted"]), ("SETTLEMENT · settlers-round", C["cream"])], "2 SETTLED · 0 AWAKE",
    ...
    if busy:
        ...
        hud.caption(img, "concept render · names are examples · settlers-round", "topdown · seed 4471 · 40x14 of 256x256 tiles · 18:07 evening")
    else:
        hud.panel(img, (g, y0, g + pw, y1), "COLONY", [
        ...
        hud.caption(img, "concept render · names are examples · settlers-round", "topdown · seed 4471 · 40x14 of 256x256 tiles · 06:41 dawn")
    return img
...
if __name__ == "__main__":
    ...
    for i, n in enumerate(extra):
        ...
        for f in creatures.FRAMES:
            creatures.frame(n, creature_tier(n), f)
