"""hud.py - HUD constants and drawing helpers for the SETTLEMENT world (canonical art module).

Principle: text lives in the frame around the world, not on it. The world region (1280x440) carries only
show-on-speak name pills (>= 20 px) with the settler's head portrait, and settler emotes. Everything else sits in an
opaque header (64 px) and a translucent bottom band (192 px) whose panels are dark, warm and rounded so the world
glows through, plus a 24 px caption strip.

Fonts (all from /System/Library/Fonts): Arial Rounded Bold for display, Verdana / Verdana Bold for body and pills.
Every text size is >= 20 px (SIZES) so it survives a 3 Mbps encode and the 320x180 thumb still shows the header shape.

    from stream.world.art import hud
    hud.header(img, left, centre, right_top, right_bottom, live=True, progress=0.4)
    hud.panel(img, (x0, y0, x1, y1), "COLONY", lines, right=hud.minimap(...))
    p = hud.pill("@kai_dnb", icon=creatures.head_icon("kai_dnb", 24), colour=creatures.palette("kai_dnb")["main"])
    hud.caption(img, left_text, right_text)
"""
...
LAYOUT: Dict[str, int] = {
    ...
    "pill_h": 34, "pill_icon": 24, "pill_gap_above_head": 6,
}
_FONTS: Dict[Tuple[str, int], ImageFont.FreeTypeFont] = {}
assert min(SIZES.values()) >= MIN_TEXT_PX
...
def font(kind: str = "body", size: Optional[int] = None) -> ImageFont.FreeTypeFont:
    ...
    if size < MIN_TEXT_PX:
        raise ValueError("HUD text must be >= %d px (asked for %d)" % (MIN_TEXT_PX, size))
    key = (path, size)
...
def pill(text: str, dot: Optional[Tuple[int, int, int]] = None, tail: bool = True, icon: Optional[Image.Image] = None,
    ...
    settler's HEAD PORTRAIT on the left (or a colour dot when no icon is given) and a small tail pointing down at them."""
...
def place_pills(pills: Sequence[Tuple[np.ndarray, int, int]], width: int) -> List[Tuple[int, int]]:
    """Stack show-on-speak pills so none overlaps: each entry is (pill_rgba, anchor_x, top_y) where top_y is the y
    the pill's bottom should touch (the settler's head top minus the gap). Returns (x, y) per pill, pushed upward
    when they would overlap an earlier one; pills are processed in the order given (sort by settler y first)."""
    placed: List[Tuple[int, int, int, int]] = []
    out = []
    for p, ax, top in pills:
        pw, ph = p.shape[1], p.shape[0]
        px = int(np.clip(ax - pw // 2, 4, width - pw - 4))
        py = max(2, top - ph + 2)
        for _ in range(8):
            hit = [q for q in placed if not (px + pw < q[0] or q[0] + q[2] < px or py + ph < q[1] or q[1] + q[3] < py)]
            if not hit:
                break
            py = min(hit, key=lambda q: q[1])[1] - ph - 2
        py = max(2, py)
...
def style_sheet_image() -> Image.Image:
    """A one-look reference of the HUD system: header, panel, pill, bubble, fonts and sizes."""
    from . import creatures
    img = Image.new("RGB", (1280, 420), (96, 150, 78))
    header(img, [("atleastonce", COLOURS["muted"]), ("SETTLEMENT", COLOURS["cream"])], "14 SETTLED · 12 AWAKE",
    ...
    panel(img, (16, 90, 440, 250), "COLONY", [[("14 settled · ", COLOURS["cream"]), ("12 awake", COLOURS["ok"])],
                                              ...
                                              [("last event: rain · by ", COLOURS["muted"]), ("@sami", (250, 204, 84))]])
    p = pill("@kai_dnb", icon=creatures.head_icon("kai_dnb", LAYOUT["pill_icon"]), colour=creatures.palette("kai_dnb")["main"])
